...
def objective(trial: Trial):
    global entropy_coef, penalty_coef, GAMMA
    lr = trial.suggest_loguniform('lr', 1e-5, 1e-1)
    entropy_coef = trial.suggest_uniform('entropy_coef', 0.001, 0.1)
    penalty_coef = trial.suggest_uniform('penalty_coef', 0.9, 0.99)
...
class Net(nn.Module):
    ...
    def loss_func(self, s, a, v_t, old_probs):
        ...
        entropy_loss = -entropy_coef * entropy.mean()  # You can adjust the entropy coefficient (e.g., 0.01)

        # Calculate penalty term
        kl_divergence = (old_probs * (old_probs.log() - F.log_softmax(logits, dim=-1))).sum(dim=1)
        penalty = penalty_coef * kl_divergence.mean()
...
if __name__ == "__main__":
    ...
    best_penalty_coef = study.best_params['penalty_coef']
    ...
    print(f"Best Hyperparameters: LR={best_lr}, Entropy Coef={best_entropy_coef}, Penalty Coef={best_penalty_coef}, GAMMA={best_GAMMA}")
