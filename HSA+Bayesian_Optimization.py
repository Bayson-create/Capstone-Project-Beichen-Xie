...
def objective(trial: Trial):
    global GAMMA, MAX_EP, MAX_EP_STEP, UPDATE_GLOBAL_ITER
    lr = trial.suggest_loguniform('lr', 1e-5, 1e-1)
    GAMMA = trial.suggest_uniform('GAMMA', 0.9, 0.99)
    # UPDATE_GLOBAL_ITER = trial.suggest_int('UPDATE_GLOBAL_ITER', 5, 10)
    # MAX_EP = trial.suggest_int('MAX_EP', 3000, 30000)
    # MAX_EP_STEP = trial.suggest_int('MAX_EP_STEP', 200, 2000)

    gnet = Net(N_S, N_A)
    gnet.share_memory()
    
    opt = SharedAdam(gnet.parameters(), lr=lr, betas=(0.92, 0.999))
    ...
    # The objective is to maximize the mean episode reward
    return -np.mean(res)
...
if __name__ == "__main__":
    # Create a study and set up the hyperparameter optimization
    sampler = TPESampler(seed=42)  # You can adjust the seed
    study = create_study(direction='maximize', sampler=sampler)
    study.optimize(objective, n_trials=20, n_jobs=5)  # You can adjust the number of trials

    # Get the best hyperparameters
    best_lr = study.best_params['lr']
    best_GAMMA = study.best_params['GAMMA']
    # best_UPDATE_GLOBAL_ITER = study.best_params['UPDATE_GLOBAL_ITER']
    # best_MAX_EP = study.best_params['MAX_EP']
    # best_MAX_EP_STEP = study.best_params['MAX_EP_STEP']

    gnet = Net(N_S, N_A)
    gnet.share_memory()
    
    opt = SharedAdam(gnet.parameters(), lr=best_lr, betas=(0.92, 0.999))
    ...
    final_mean_reward = np.mean(res)
    
    # print(f"Best Hyperparameters: LR={best_lr}, GAMMA={best_GAMMA}, MAX_EP={best_MAX_EP}, MAX_EP_STEP={best_MAX_EP_STEP}, UPDATE_GLOBAL_ITER={best_UPDATE_GLOBAL_ITER}")
    print(f"Best Hyperparameters: LR={best_lr}, GAMMA={best_GAMMA}")
    print(f"Final Mean Reward with Best Hyperparameters: {final_mean_reward}")
